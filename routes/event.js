...
route.post("/", (req,res)=>{
    ...
    .then((event)=>{
        ...
        // Create a new Discussion for the event with this event's id in event
        models.Discussion.create({
            event: event._id
        })
        .then((discussion)=>{
            // If Discussion created successfully, Save current discussion's Id in Event
            event.discussion = discussion._id;
            return event.save();
        })
        .then((event)=>{
